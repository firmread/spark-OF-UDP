...
void ofApp::update(){
	...
	char udpMessage[sizeof(returnPacket)];
    memset(udpMessage, 0, sizeof(returnPacket));
    int nBytesRevd = udp.Receive(udpMessage, sizeof(returnPacket));
    
    
	if(nBytesRevd == sizeof(returnPacket)){
        bGotSth = true;
        
        memset(&rp, 0, sizeof(returnPacket));
        memcpy(&rp, udpMessage, sizeof(returnPacket));
